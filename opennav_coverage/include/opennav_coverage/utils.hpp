...
namespace util {
        ...
        const std_msgs::msg::Header& header, const bool is_cartesian)
    {
        ...
        for (unsigned int i = 0; i != path.size(); i++) {
            ...
            if (path.getState(i).type != PathSectionType::SWATH && path.getState(i).type != PathSectionType::TURN) {
        ...
        const float& pt_dist)
    {
        ...
        for (unsigned int i = 0; i != path.size(); i++) {
            ...
            if (i > 0 && path.getState(i).type == PathSectionType::SWATH && path.getState(i - 1).type == PathSectionType::SWATH) {
                ...
                pose.pose.orientation = nav2_util::geometry_utils::orientationAroundZAxis(path.getState(i).angle);
